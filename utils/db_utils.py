...
class Base(object):
    ...
    def update(self, collection_name, query, value):
        try:
            if isinstance(value, list):
                result = self.mongo_db[collection_name].update_many(
                    query, value)
            else:
                result = self.mongo_db[collection_name].update_one(
                    query, value)
            return result
        except Exception as e:
    ...
    def delete(self, collection_name, query):
        try:
            self.mongo_db[collection_name].update_many(
                query, {"$set": {"meta.is_deleted": True}})
        except Exception as e:
